...
def collect_dict(path, split, replace_videos):
    ...
    print("start scan!")
    filenames = os.listdir(str(split_video_path))
    for filename in tqdm(filenames, total=len(filenames)):
        if filename.lower().endswith(".avi"):
            split_videos.append(Path(os.path.join(str(split_video_path), filename)))
...
def main(path):
    ...
    replace = list((path / 'k400/replacement').rglob('*.avi'))
...
if __name__ == '__main__':
    path = "/mnt/home/hexiang/kinetics-dataset/"
    main(path)