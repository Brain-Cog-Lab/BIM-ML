...
def extract_wav_from_avi():
    for split in splits:
        ...
        kinetics_sound_vids = load_csv(str(kinetics_400_dir / "my_{}.txt".format(split)))
...
def extract_frames_from_avi():
    for split in splits:
        ...
        for root, dirs, files in os.walk(kinetics_sound_video_test_dir):
            for filename in files:
                if filename.lower().endswith(".avi"):
                    # 拼接文件路径并转换为 Path 对象
                    if filename[:11] in kinetics_sound_vids:
                        video_src_paths.append(Path(root) / filename)
