...
@dataclass
class RandomChoiceDiskCachedDataset:
    """
    RandomChoiceDiskCachedDataset caches the data samples to the hard drive for subsequent reads, thereby potentially improving data loading speeds.
    If dataset is None, then the length of this dataset will be inferred from the number of files in the caching folder. Pay
    attention to the cache path you're providing, as DiskCachedDataset will simply check if there is a file present with the index that
    it is looking for. When using train/test splits, it is wise to also take that into account in the cache path.
    ...
    def __getitem__(self, item) -> Tuple[object, object]:
        ...
        file_path = os.path.join(self.cache_path, f"{item}_{copy}.hdf5")
        try:
            data, targets = dir_load_from_disk_cache(file_path)
        except (FileNotFoundError, OSError) as _:
            logging.info(
                f"Data {item}: {file_path} not in cache, generating it now",
                stacklevel=2,
            )
            visual_path, targets = self.dataset[item]

            dir_save_to_disk_cache(
                visual_path, targets, file_path=file_path, compression=self.compression
            )
            # format might change during save to hdf5,
            # i.e. tensors -> np arrays
            data, targets = dir_load_from_disk_cache(file_path)
...
def dir_save_to_disk_cache(
    ...
    # with h5py.File(file_path, "w") as f:
        for name, data in zip(["data", "target"], [data, targets]):
            if name == "data":
                dir_data = ()  # empty tuple
                path1 = []
                for i in range(file_num):
                    path1.append('frame_' + str(i) + '.jpg')
                    if i in [1, 4, 7]:
                        dir_data += (np.array(PIL.Image.open(data + "/" + path1[i]).convert('RGB')), )
                data = dir_data
            else:
                if type(data) != tuple:
                    data = (data,)
...
def dir_load_from_disk_cache(file_path: Union[str, Path]) -> Tuple:
    """
