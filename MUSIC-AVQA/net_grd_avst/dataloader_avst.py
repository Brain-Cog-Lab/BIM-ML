...
class AVQA_dataset(Dataset):
    ...
    def __init__(self, label, audio_dir, video_res14x14_dir, transform=None, mode_flag='train'):
        ...
        samples = json.load(open('/mnt/home/hexiang/MUSIC-AVQA/data/json/avqa-train.json', 'r'))
    ...
    def __getitem__(self, idx):
        ...
        # 使用 h5py 读取
        with h5py.File(os.path.join(self.video_res14x14_dir, name + '.h5'), 'r') as hf:
            # 从名为 "features" 的数据集中读取
            visual_posi = hf["features"][::6, ...]  # shape: (60, 512, 14, 14)
        ...
        for i in range(visual_posi.shape[0]):
            ...
            with h5py.File(os.path.join(self.video_res14x14_dir, neg_video_name + '.h5'), 'r') as hf_nega:
                visual_nega_out_res18 = hf_nega["features"][neg_frame_flag]  # shape: (60, 512, 14, 14)
            ...
            visual_nega_clip = visual_nega_out_res18.unsqueeze(0)
