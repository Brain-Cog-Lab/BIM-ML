...
class AVQA_Fusion_Net(nn.Module):
    ...
    def __init__(self):
        ...
        # inverse
        self.audio_fc = nn.Linear(512, 42)
        self.visual_fc = nn.Linear(512, 42)
    ...
    def forward(self, audio, visual_posi, visual_nega, question):
        ...
        audio_feat = audio_feat_att+audio_feat_be.mean(dim=-2).squeeze()
        visual_feat = visual_feat_att+visual_feat_grd_be.mean(dim=-2).squeeze()
        ...
        audio_feature = torch.mul(audio_feat, qst_feature)
        # combined_feature = self.tanh(combined_feature)
        a_out_qa = self.audio_fc(audio_feature)              # [batch_size, ans_vocab_size]

        visual_feature = torch.mul(visual_feat, qst_feature)
        # combined_feature = self.tanh(combined_feature)
        v_out_qa = self.visual_fc(visual_feature)              # [batch_size, ans_vocab_size]
        ...
        return a_out_qa, v_out_qa, out_qa, out_match_posi,out_match_nega
