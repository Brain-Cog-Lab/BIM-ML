...
# 定义要提取的时间点（秒）
times = [1, 5, 10, 15, 20]  # 提取第1秒，第5秒，第10秒，第15秒和第20秒的帧

cap = cv2.VideoCapture(video_path)
vid_len = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))

frame_interval = int(vid_len / 10)

sample_num = 1
frame_num = video_frame_sample(frame_interval, 10, sample_num)
...
while True:
    ...
    if frame_count in frame_num:
        # 保存当前帧
        cv2.imwrite(os.path.join("/root/ave", f'frame_{frame_count}.jpg'), pre_frame)
    frame_count += 1
